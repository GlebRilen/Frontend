...
export const checkPatternComplete138 = async function(fetchBuil, el, coin){
    // try{
    ...
    if(stage === 100 && P5I >= 0) {
        for(let t = idx; t < arr.length; t++){
            if(arr[t][2] > el.top) el.data.confirm = true //Checking - was pattern confirmed by passing break line or not. If pass break line - then SL; if not - then cancell order.
            if(arr[t][3] < SL) {
            ...
            if(arr[t][2] >= T1){
                ...
                await objectRebuild(arr, P5I, el, t, 'T', '138'/*261*/, coin)
